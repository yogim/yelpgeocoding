...
public class GeocodingUtils {
	...
	public static LocationCoordinateModel getLocationCordinateFromAddress(String address) {
		...
		try {
			...
			Object lat = (double)jsonObj.getJSONArray("results").getJSONObject(0).getJSONObject("geometry")
					.getJSONObject("location").get("lat");
			Object lng = (double)jsonObj.getJSONArray("results").getJSONObject(0).getJSONObject("geometry")
					.getJSONObject("location").get("lng");

			locCordinateModel.setLangitude((double)lat);
			locCordinateModel.setLattitude((double)lng);
			conn.disconnect();
